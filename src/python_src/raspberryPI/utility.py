...
def generate_verilog(fc1_W, fc2_W, norm1_g, norm2_g, output_path='bitnet_mini.v'):
    ...
    a("//")
    a("// NOTE: Multi-logit ports are flattened into single packed buses")
    a("// (e.g. \"output wire signed [159:0] logit_out\") instead of")
    a("// unpacked array ports (e.g. \"[15:0] logit_out [0:9]\"), since the")
    a("// latter is SystemVerilog-only and rejected by Yosys's default")
    a("// Verilog-2001 frontend. Each 16-bit logit i is accessed via the")
    a("// indexed part-select logit_out[i*16 +: 16].")
    a("// ============================================================")
    a("")
    a("`timescale 1ns/1ps")
    a("")
    ...
    a(f"    input  wire [{N_IN-1}:0]        pixel_in,   // {N_IN} binary pixels, LSB = pixel[0]")
    a(f"    output wire signed [{N_OUT*16-1}:0] logit_out   // {N_OUT} raw logits, flattened: logit_out[i*16 +: 16]")
    a(");")
    a("")
    ...
    if NORM2_SHIFT > 0:
        a(f"            assign logit_out[gn2*16 +: 16] = fc2_raw[gn2] >>> {NORM2_SHIFT};")
    else:
        a(f"            assign logit_out[gn2*16 +: 16] = fc2_raw[gn2];")
    a(f"        end")
    a(f"    endgenerate")
    a("")
    a("endmodule")
    ...
    a("// ============================================================")
    a("module argmax10 (")
    a(f"    input  wire signed [{N_OUT*16-1}:0] logit_in,   // {N_OUT} x 16-bit logits, flattened: logit_in[i*16 +: 16]")
    a("    output reg  [3:0]          digit_out   // 0-9")
    a(");")
    a("    integer i;")
    a("    reg signed [15:0] max_val;")
    a("    always @(*) begin")
    a("        max_val   = logit_in[0 +: 16];")
    a("        digit_out = 4'd0;")
    a(f"        for (i = 1; i < {N_OUT}; i = i + 1) begin")
    a("            if ($signed(logit_in[i*16 +: 16]) > max_val) begin")
    a("                max_val   = logit_in[i*16 +: 16];")
    a("                digit_out = i[3:0];")
    a("            end")
    a("        end")
    a("    end")
    ...
    a(f"    input  wire [{N_IN-1}:0] pixel_in,")
    a("    output wire [3:0]   digit_out")
    a(");")
    a(f"    wire signed [{N_OUT*16-1}:0] logits_flat;")
    a("    bitnet_mini  nn  (.pixel_in(pixel_in), .logit_out(logits_flat));")
    a("    argmax10     ax  (.logit_in(logits_flat), .digit_out(digit_out));")
    a("endmodule")
    a("")
